...
而对于水平触发，事件一旦触发，需要用户一直读，直到缓冲区数据全部读完为止才可以，如果设置为block，在没有数据可读时，就block了，需要用eagain错误来告知用户，数据读完了，不用再读了。
 */
...
int main (int argc, char* argv[])
{
    ...
    assert(epollfd != -1);
    addfd(epollfd,listenfd,true);// 把监听套接字存放到epoll true采用ET模式                  //epoll_ctl
